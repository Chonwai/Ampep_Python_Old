    ...
    def training(self, fold=10, trees=100, method="KFold"):
        for h in range(1):    
            for i in range(1):
                ...
                cv = self.router(method, fold + 10 * h)

                # score = cross_validate(clf, self.X, self.y, cv=cv, n_jobs=-1, scoring=('accuracy'))
                print("Finished Training Model " + str(i) + " Times with " + str(fold + 10 * h) + " Fold and " + str(trees + 100 * i) + " Trees!")
                model = './model/' + method + '/RF_' + method + '_' + str(fold + 10 * h) + '_' + str(trees + 100 * i) + '.pkl'

                with open(model, 'wb') as file:
                    pickle.dump(clf, file)

                # self.calculateScore(score, fold + 10 * h, scoring=['accuracy'])
